...
use super::{Result, Cpu, Sreg, Reg16, Config};

pub fn emulate(file: &str) -> Result<()> {
    let cfg = Config {
        bios_file: PathBuf::from(file),
        bios_addr: 0x0000_0000,
    };

    let mut cpu = Cpu::new(&cfg)?;

    // initialize registers
    cpu.write_sreg(Sreg::CS, 0x0000); // todo
    cpu.write_ip((0x0000) as u16); // todo

    cpu.write_flags(0);

    cpu.write_sreg(Sreg::DS, 0x1000); // todo
    cpu.write_sreg(Sreg::ES, 0x1000); // todo
    ...
    cpu.write_reg16(Reg16::SP, 0xFFFE);
